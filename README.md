...
*class* [Interaction](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Interaction.java)
...
Each instrument maintains a list of changes specific to that instrument. Interactions that originate from these changes can be instantaneous, durational, or repetitive as the [Action](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Action.java) and [Reaction](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Reaction.java) type classifications also indicate.

---

*@interface* [Change](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Change.java)

The `Change` annotation type represents the classification of interactions in performance. It is the nucleus for all interactions and can be permuted to generate individual instances of interactions that define the atomic pieces for performing a musical instrument. Each change can be classified in three contexts: [Effect](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Effect.java), [Action](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Action.java), and [Reaction](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Reaction.java). There are multiple categories in each classification and a target class of the `Change` annotation can be of one or more categories in each of the three contexts. These categories are described per instrument in the [original document](https://github.com/exbuddha/Solar/tree/master/doc/Original.md).
...
The axioms are defined as enum types [Action](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Action.java), [Reaction](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Reaction.java), and [Effect](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Effect.java). Wherever applicable, variations to these types can be included as new interfaces in order to fine-tune the three categories, each well-defined in the context of their specific instrument and performer types. Otherwise, the existing enums must be expanded to provide a wider range of options to the end user.
...
*interface* [State](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/State.java)
...
[Description](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Description.java) is used all throughout this mentioned configuration to explain and resolve ambiguities for instantaneous interactions or technical configurations.
...
*interface* [Snapshot](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Snapshot.java)
...
*class* [Performer](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Performer.java).PerformanceGraph
...
Snapshots are, again, essentially smaller collections of references to the `PerformanceGraph` data points (edges and vertices). Whenever these points are fixed in time or carry some notion of time involved, they can be defined as snapshot instances of something that is produced. Currently `Snapshot.Instance<T>` defines `T` as an arbitrary object and doesn't enforce it to be inherited from [Production](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Production.java), but this might becomes useful later on. If one imagines two adjacent vertices in the `PerformanceGraph` as two points in time, the snapshot instances can represent mid-way points by pointing at both vertices and providing a point in time relative to both. This way all situations can effectively be pointed at.

---

*class* [Conductor](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Conductor.java)

[Conductor](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Conductor.java) through the use the `Score.Conductor` and the data structured in the available `Conductor.Coordination` class prepares an `Conductor.Orchestration` type. This confirms `Orchestration` being a subclass of `Coordination`.
...
*interface* [Preference](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Preference.java)

Preferences are designed as methods that return collections of graph edges along with specific data types that will aid the model in scoring those edges for a variety of purposes, ultimately reducing the performance graph into a single path. This path is simply named performance and is represented by the `Conductor.Performance` class as the end product of this solution. [Instruction](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Instruction.java) types can be written to disk as final representation of performance paths forming the promised end-result of the entire workflow.

The `Preference.Method` interface is the base type for all such preferences that are applied to graph structure. To leave open the possibility for any alternative design for preferences that cannot follow this form, the `Preference` interface itself will be kept empty or as general as possible.

`Preference` is the most open-ended interface with many forms and many interfaced relations for conversions involved in populating or traversing performance graphs. They can also be used in any other form to discern code paths for making human-like decisions during the entire process of translating from score to performance data. Because preferences are defined as methods that can accept any argument, this allows them to be completely aware of the model. Performers and the conductor can follow a simple paradigm of applying all preferences at every iteration given that these preference methods are able to discern quickly if, or if not, they are applicable. Added to this approach, a performer and the conductor can become aware of the performance graph by the use of special functionality to narrow down the number of preferences that are applicable at any time. Some preferences are set at the start of the entire process of creating instructions and they will stick all the way to the end, such as age-related or instrument-specific preferences. Many others exist only to enforce anatomical rules if the `ChangeGraph` is known to produce some noise or unwanted paths in the `PerformanceGraph`. Ultimately, it is by understanding the [State](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/State.java).Graphs related to each `Snapshot.Instance` within `Orchestration` that each [Performer](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Performer.java) gets the change to exert the appropriate set of preferences.

---

*class* [Instruction](https://github.com/exbuddha/Solar/blob/master/java/musical/performance/Instruction.java)
...
---
  ...
  > Alireza Kamran - akamran@gmail.com
