...
It is many times during listening closely that whenever the movement of melody in a phrase traverses over a tightly closed interval relative to the entire intervallic mood (key and chord), as in stepping up and down by half- or a full-tone, the opposite directional movement in that case carries qualities such that when the original melody is repeated again, or is complemented, then the rest of the phrase brings in a taste that is pleasantly felt. Similarly so, there is consensus among composers that when sub-phrases become more densely centered around closed intervals the memory forms a habit of wanting to hear a similar relational occurrence among tonalities that will be remembered from the earliest available point in memory.
...
The benefit of rules of inference is that they are infinitely expansible. This fits their purpose greatly. Laws and facts originate from physical rules of contact and motion, anatomical sensations and preferences, sonic preferences, instrument-specific rules of performance (the most determining and valuable one), and score demands (the strongest eliminating factor). Some of these are included in the change graph, which I will introduce later on in code, but essentially their existence is tied to rules of inference specific to human anatomical features and that of the instruments. Many others are exerted on available paths that are collectively called preferences.

---

### Music Performance

Because it is the human body that acts upon musical instruments during performance, in order to devise a scientific formulation for automating the subject entirely, effort must be placed on finding balanced pieces that can describe real events with the right angle of view such that it allows the mind and any computing unit to efficiently and correctly analyze them with respect to time in a music score. This correctness is the keyword that makes the previous statement sound like a paradox and, at the same time, sits at the center of the challenge at hand. The goal of the code project presented with this conceptual paper is to define such a correctly angled system where music data scribed in standard documented notations can be be transformed into individual, well-defined, and humanly understandable events from which readable instructions can be derived. These events ultimately will have to represent an abstraction of the physical world in a scientific language that covers a span from the laws of physics, mechanics, dynamics and statics, and thermodynamics, to that of electronics, sound, and optics. Therefore, the scope of performance analysis can be a gigantically large one. Similar to any other simulation system, the language of abstraction determines how that system behaves and adapts to the changes that are ultimately governed by external constants: in this case, the music score. How granular a system aims to model the physical world in which objects of performance reside is a matter of choice and attention to details. In this project, I decided to set the abstraction level paralleled with units of action that are meaningful in the context of performance for each individual instrument. Therefore, there are not many classes dealing with motion types and their physical properties such as force or velocity yet. There also are not many classes that deal with the properties of the three-dimensional space as studies on geometrical forms and the formulas that aid in finding critical parts of those forms would cover. As a result, the source code assumes that each performer type is aware of the details of actions it can take in order to properly play an instrument and therefore hides those details as soon as they prove to be overly fine-grained. At the same time, all instrument types are made aware of the states that their smaller parts will be in after a performer takes action on them. A certain amount of normalization is applied to the movements that performers make, the reactions that instrument will show, and the effects that are emitted in the aural field as sound. This normalized unit of data is called interaction and it forms the basis for the decision making that stands in the way of generating human instructions from the data recoded in music scores. In the following paragraphs, I will introduce these interactions and other forms of data that interactions relate to.

The [performance](https://github.com/exbuddha/Solar/tree/master/java/performance) package contains a number of high-level definitions that collectively aim to solve the problem of music performance for all instrument types in music. There has been an active effort, during the design phase of the project, to set the level of generality such that allows programmers to simulate more than just musical instruments and performers. This means that similar paradigms can be followed in order to design solutions for any form of performance simulation, such as game characters or electronic and computational parts of circuits. However, those are outside of the scope of this project so the names given to classes in this package are only useful for the purpose of music performance but the higher order among them can be re-imaged for almost all other types of automation.

The rest of this chapter is spawned from its previous version. The old document was written as a first attempt to draw out a large-scale view of an algorithmic approach to solving the problem at hand; therefore, it is a little out-dated and is filled with questions or points that were open for discussion. Since the time of publishing that paper, I have made the effort to answer all the question which I posed to myself and the reader at that time. The document is linked below and I still consider it to be a fine article for any follower who is interested in working on those questions or in being aware of the flow of the growth of the project. I believe that the reader is better off completing the old document before continuing with the rest of this paper since this new writing is essentially a short overview of that document added a new flow chart and a short description of the final algorithm. Then, hopefully the rest of this paper will start to make more sense.

At any time during studying this chapter, if questions seem to become unexplainable or be left unanswered, I urge you to return to the source code and contemplate on methodologies presented in there. This work is still under design; therefore, the code can only present you with a large-scale views or the conceptual models of final class definitions. Many classes and methods are still left empty and there are a far larger number of new relations that must be introduced in order to complete the work. As time passes and as I get the chance to revisit my work, I will attempt to present the complete picture. A recent evaluation has convinced me that the scope of the project has reached a point passed individual capabilities of myself and very likely others as well. Continuing further implies that the presented functionality must be tuned to support full applications which require a large team of engineers and much more organized efforts. I invite every scientist and enthusiastic stakeholder who sees the potential in, and can provide the financial support for, forming and managing a team of engineers to take on the role of leadership on their own and run with the project as they desire. My intention has always been, and still is, to offer this work publicly as an open-source project for the educational benefits that it carries.

The original document can be found [here](https://github.com/exbuddha/Solar/tree/master/doc/Original.md). Keep in mind that the original document introduces the current data types slightly differently. The following sections reiterate or recapture the same content in a more relevant language that conforms to the latest version of the source code.

---

*class* [Interaction](https://github.com/exbuddha/Solar/blob/master/java/performance/Interaction.java)

Interactions are individual instances of change that each pair of instrument and performer can produce. All interaction instances define their own specific masculine and feminine parts, or any other data specific to the interaction. A masculine part is the body part, accessory, or an instrument part that is used to initiate an action on the musical instrument in order to create or change the sound or to make other interactions on that instrument possible. A feminine part is the part (or body) of the musical instrument that is acted upon by the masculine part and reacts to create or change sound. It is important to keep in mind that interaction are generally free from the time axis. This means that they don't point to an exact point in time as physical events do but rather define an span in time which the action-reaction-effect tri-ordinate can occupy.

Each instrument maintains a list of changes specific to that instrument. Interactions that originate from these changes can be instantaneous, durational, or repetitive as the [Action](https://github.com/exbuddha/Solar/blob/master/java/performance/Action.java) and [Reaction](https://github.com/exbuddha/Solar/blob/master/java/performance/Reaction.java) type classifications also indicate.

---

*@interface* [Change](https://github.com/exbuddha/Solar/blob/master/java/performance/Change.java)

The [Change](https://github.com/exbuddha/Solar/blob/master/java/performance/Change.java) annotation type represents the classification of interactions in performance. It is the nucleus for all interactions and can be permuted to generate individual instances of interactions that define the atomic pieces for performing a musical instrument. Each change can be classified in three contexts: [Effect](https://github.com/exbuddha/Solar/blob/master/java/performance/Effect.java), [Action](https://github.com/exbuddha/Solar/blob/master/java/performance/Action.java), and [Reaction](https://github.com/exbuddha/Solar/blob/master/java/performance/Reaction.java). There are multiple categories in each classification and a target class of the [Change](https://github.com/exbuddha/Solar/blob/master/java/performance/Change.java) annotation can be of one or more categories in each of the three contexts. These categories are described per instrument in the [original document](https://github.com/exbuddha/Solar/tree/master/doc/Original.md).

Up to this point, given all the descriptions above, an interaction instance can be visualized as a three-dimensional data point on the axes of action, reaction, and effect types and with a relaxed axis of time.

![Interaction Axioms](https://github.com/exbuddha/Solar/blob/master/doc/img/Performance-1.png)

The axioms are defined as enum types [Action](https://github.com/exbuddha/Solar/blob/master/java/performance/Action.java), [Reaction](https://github.com/exbuddha/Solar/blob/master/java/performance/Reaction.java), and [Effect](https://github.com/exbuddha/Solar/blob/master/java/performance/Effect.java). Wherever applicable, variations to these types can be included as new interfaces in order to fine-tune the three categories, each well-defined in the context of their specific instrument and performer types. Otherwise, the existing enums must be expanded to provide a wider range of options to the end user.

---

*class* [Performer](https://github.com/exbuddha/Solar/blob/master/java/performance/Performer.java).ChangeGraph

Change graphs record the interaction space of instruments and performers. They carry enough data in enough details that will allow the action-selection logic to discern how to correctly and properly move through a large number of possibilities at performance time. Their main focus point (vertices) is interactions.

---

*interface* [State](https://github.com/exbuddha/Solar/blob/master/java/performance/State.java)

States are well-defined normalized objects that describe the anatomical forms of the performer body and that of the instrument at any given instance of time during performance. States are statically defined data points collectively setting the domain of the possibility space for parts and their instruments. They are additional data in the scheme explained here that help action-selection fine-tune the decision making to less number of edges at any given instance in performance.
...
[Description](https://github.com/exbuddha/Solar/blob/master/java/performance/Description.java) is used all throughout this mentioned configuration to explain and resolve ambiguities for instantaneous interactions or technical configurations.
...
There is some amount of redundancy in having both `ChangeGraph` and state graph in the design currently and this needs to be carefully resolved. The state graph needs some more focused study. As it is currently obvious, from reading the last two lines, state graph is not very well-defined at all. It is not exactly clear what type of edges this graph will represent. They must be a form of interaction type but this turn the state graph into an equivalent of `ChangeGraph` for which the vertices and edges are swapped.

---

*interface* [Snapshot](https://github.com/exbuddha/Solar/blob/master/java/performance/Snapshot.java)

Snapshots deal solely with states of performers and their instruments through time. Therefore, they are in charge of handling real time events that are mapped to states. These temporal types will retain a connection to the durations that are parts of `Score.Conductor` logic. Snapshots wrap around timed states and the `PerformanceGraph` paths to maintain a physically explainable description of performer interactions and their anatomical relationships.

States, therefore, require to know about their orchestration snapshots for backward probing by performer logic during performance.

---

*class* [Performer](https://github.com/exbuddha/Solar/blob/master/java/performance/Performer.java).PerformanceGraph

The `PerformanceGraph` consists of all interactions that occupy the entire possibility space in performance. As time passes and as more interactions become eligible, those instances can be connected to each other to form instruction lines. Each line then will be evaluated based on the preferences that are available for that specific instance on the instruction line. After every evaluation the score/weight of that line changes and this, in effect, makes that line of instruction more or less possible, desirable, or preferred.

Snapshots are, again, essentially smaller collections of references to the `PerformanceGraph` data points (edges and vertices). Whenever these points are fixed in time or carry some notion of time involved, they can be defined as snapshot instances of something that is produced. Currently [Snapshot](https://github.com/exbuddha/Solar/blob/master/java/performance/Snapshot.java).Instance<T> defines `T` as an arbitrary object and doesn't enforce it to be inherited from [Production](https://github.com/exbuddha/Solar/blob/master/java/performance/Production.java), but this might becomes useful later on. If one imagines two adjacent vertices in the `PerformanceGraph` as two points in time, the snapshot instances can represent mid-way points by pointing at both vertices and providing a point in time relative to both. This way all situations can effectively be pointed at.

---

*class* [Conductor](https://github.com/exbuddha/Solar/blob/master/java/performance/Conductor.java)

[Conductor](https://github.com/exbuddha/Solar/blob/master/java/performance/Conductor.java) through the use the `Score.Conductor` and the data structured in the available [Conductor](https://github.com/exbuddha/Solar/blob/master/java/performance/Conductor.java).Coordination class prepares an [Conductor](https://github.com/exbuddha/Solar/blob/master/java/performance/Conductor.java).Orchestration type. This confirms `Orchestration` being a subclass of `Coordination`.

`Orchestration`, in turn, generates the internal structures required for handling performance events that the performer will later on emit. This process is made available by what the performers of that score will need to record their detailed movements during the performance. `Orchestration` then deals with snapshots and their configuration.

The chart below describes the relations among the classes that are discussed up to this point.

![Music Conduction](https://github.com/exbuddha/Solar/blob/master/doc/img/Performance-2.png)

---

*interface* [Preference](https://github.com/exbuddha/Solar/blob/master/java/performance/Preference.java)

Preferences are designed as methods that return collections of graph edges along with specific data types that will aid the model in scoring those edges for a variety of purposes, ultimately reducing the performance graph into a single path. This path is simply named performance and is represented by the [Conductor](https://github.com/exbuddha/Solar/blob/master/java/performance/Conductor.java).Performance class as the end product of this solution. [Instruction](https://github.com/exbuddha/Solar/blob/master/java/performance/Instruction.java) types can be written to disk as final representation of performance paths forming the promised end-result of the entire workflow.

The [Preference](https://github.com/exbuddha/Solar/blob/master/java/performance/Preference.java).Method interface is the base type for all such preferences that are applied to graph structure. To leave open the possibility for any alternative design for preferences that cannot follow this form, the [Preference](https://github.com/exbuddha/Solar/blob/master/java/performance/Preference.java) interface itself will be kept empty or as general as possible.

[Preference](https://github.com/exbuddha/Solar/blob/master/java/performance/Preference.java) is the most open-ended interface with many forms and many interfaced relations for conversions involved in populating or traversing performance graphs. They can also be used in any other form to discern code paths for making human-like decisions during the entire process of translating from score to performance data. Because preferences are defined as methods that can accept any argument, this allows them to be completely aware of the model. Performers and the conductor can follow a simple paradigm of applying all preferences at every iteration given that these preference methods are able to discern quickly if, or if not, they are applicable. Added to this approach, a performer and the conductor can become aware of the performance graph by the use of special functionality to narrow down the number of preferences that are applicable at any time. Some preferences are set at the start of the entire process of creating instructions and they will stick all the way to the end, such as age-related or instrument-specific preferences. Many others exist only to enforce anatomical rules if the `ChangeGraph` is known to produce some noise or unwanted paths in the `PerformanceGraph`. Ultimately, it is by understanding the [State](https://github.com/exbuddha/Solar/blob/master/java/performance/State.java).Graphs related to each [Snapshot](https://github.com/exbuddha/Solar/blob/master/java/performance/Snapshot.java).Instance within `Orchestration` that each [Performer](https://github.com/exbuddha/Solar/blob/master/java/performance/Performer.java) gets the change to exert the appropriate set of preferences.

---

*class* [Instruction](https://github.com/exbuddha/Solar/blob/master/java/performance/Instruction.java)

[Instruction](https://github.com/exbuddha/Solar/blob/master/java/performance/Instruction.java) has to be the most interesting part of the design and project itself. Each instruction is, at its coarsest level of definition, tied to individual Interaction instances because every Interaction type requires a form of action from the performer. Because of this relationship and because of the time-sensitivity and dependence of instructions, every edge of the `ChangeGraph` is also connected to at most two instructions. These edges form the order among instructions, where one moves to another through time. They also provide a level of granularity to the already available instruction set for each performer-instrument type. Additionally, states which are connected through the `PerformanceGraph` to the `ChangeGraph` vertices (interactions) can provide the finest form of definition for the language of instructions.

Following the paradigm described in this writing, the entire flow of generation of instructions can be imagined and realized over time.
...
---
  ...
  > Blessings to all who will continue this work to the final point.
